...
const ContactSection = () => {
  ...
  return <section id="contact" className="px-6 py-0">
    <div className="vertical-line h-32 mb-16"></div>
    <h2 className="section-title">04 Kontakt</h2>

    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-12">
        {!isSubmitted ? <form onSubmit={handleSubmit} className="max-w-xl mx-auto space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium mb-1 text-left">Name</label>
              <Input id="name" placeholder="Dein Name" value={name} onChange={e => setName(e.target.value)} required disabled={isLoading} />
            </div>
            <div>
              <label htmlFor="email" className="block text-sm font-medium mb-1 text-left">E-Mail</label>
              <Input id="email" type="email" placeholder="Deine E-Mail" value={email} onChange={e => setEmail(e.target.value)} required disabled={isLoading} />
            </div>
          </div>
          <div>
            <label htmlFor="subject" className="block text-sm font-medium mb-1 text-left">Betreff</label>
            <Input id="subject" placeholder="Dein Betreff" value={subject} onChange={e => setSubject(e.target.value)} disabled={isLoading} />
          </div>
          <div>
